...
class DataService:
    ...
    async def set_user_keys(self, user_id: str, keys: models.SetAllKeysRequestModel) -> None:
        await self.user_keys.add_public_keys(
            user_id,
            keys.registration_id,
            keys.identity_key,
            keys.signed_key.index,
            keys.signed_key.signed_prekey,
            keys.signed_key.signature
        )
    ...
    async def get_user_data(self, user_id: int) -> models.UserResponse:
        ...
        data.avatar = f"{settings.S3_AVATARS}/avatars/{user_id}.jpeg"
        return data
    ...
    async def get_users_data(
        ...
        for user in data.users:
            user.avatar = f"{settings.S3_AVATARS}/avatars/{user.id}.jpeg"
