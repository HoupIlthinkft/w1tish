async function get_data_by_user_id(user_id) {
    const data = await fetch((window.ENV.API_URL + `/web/data/user?user_id=${user_id}`), {
        method: 'GET',
...
async function get_data_users_ids(users_ids) {
    ...
    const data = await fetch((window.ENV.API_URL + `/web/data/user?${user_id.join("&")}`), {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
    });
...
async function get_data_by_username(username) {
    const data = await fetch((window.ENV.API_URL + `/web/data/user?username=${username}`), {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
    });
