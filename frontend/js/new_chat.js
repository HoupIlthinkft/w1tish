...
async function add_user_in_invitation() {
    ...
    if (input_value != "" && document.getElementById("add_users").childElementCount < 8) {
       if (input_value != document.getElementById("nickname").textContent) { 
        ...
        } else show_notifications("К сожалению нельзя общаться со своей шизой :(", "error");
    } else show_notifications("Максильмальное кол-во приглашенных человек не более 8", "error");
}
    ...
    if (document.getElementsByClassName("added_user").length != 0) {
        ...
        for (let i = 0; i < document.getElementsByClassName("contact").length; i++){
            ...
            if (JSON.stringify(list_contact.sort()) == JSON.stringify(list_nickname.sort())) {
                show_notifications("Чат с данным набором юзеров уже существует, попробуйте по другому", "error")
            }
