...
export async function login(username, password) {
    ...
    if (response.status === 422) console.log("Виноват фронтендер");
        else if (response.status === 404) callNotification("Данного юзера не существует, зарегистрируйтесь", "error");
...
export async function getProtectedData() {
    ...
    if (accessToken != null) {
        ...
        if ((response.status === 401) || (response.status === 422)) { 
            ...
            callNotification("Пользователя с данным логином/почтой не существует, попробуйте зарегистрироваться", "error");
...
export async function get_data_by_username(username) {
    ...
    if (data.status === 200) return data.json();
        else if (data.status === 404) callNotification("Пользователя с данным username не существует", "error");
            else callNotification(`Ошибка сервера: ${data.status}`, "error");
}
