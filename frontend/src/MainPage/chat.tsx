  ...
  const sendMessage = () => {
    if (inputMessage.current.value.trim() != '') {
      send_new_message(
        ...
        profile.chats[activityChat].find((member) => member != profile.id),
        useChatStore.getState().activityChat,
      );
      inputMessage.current.value = '';
    }
  };
        ...
        <>
          ...
          <div className="mx-[clamp(5px,0.5vw,10px)] my-[clamp(5px,1vh,10px)] mr-[clamp(5px,3vw,60px)] flex flex-row items-center justify-between gap-[clamp(5px,1vw,20px)] rounded-[20px]">
            <textarea
              ...
              className="border-border bg-plate-accent max-h-[20vh] min-h-min w-[93%] rounded-[40px] border px-[clamp(5px,1vw,20px)] py-[clamp(5px,2vh,20px)] text-[clamp(0.75rem,1.5vw,1.5rem)] outline-0"
            />
            <span
