...
function registration_account() {
    if (document.getElementsByClassName("main").item(0).id == "main_registration"){
        ...
        const password = document.getElementById("input_password").value;
        const password_confirmation = document.getElementById("confirmation_input_password").value;
        const error_text = document.getElementById("error");
                ...
                if (password.toUpperCase() != password) {
                    if (password == password_confirmation) {
                        if (email.checkValidity()) {
                            register_user(username, email.value, password);
                            error_text.textContent = "";
...
function create_sign_in_container() {
    const parent = document.getElementsByClassName("main").item(0).parentNode;
    const new_main_div = document.createElement("div");
    ...
    const error_h = document.createElement("h1");
    error_h.id = "error";
...
function create_registration_container() {
    ...
    const password_confirmation_input = document.createElement("input");
    ...
    const sign_in_btn = document.createElement("p");
    sign_in_btn.id = "sign_in";
    sign_in_btn.className = "btn"
    sign_in_btn.textContent = "Войти в аккаунт";

    const registration_btn = document.createElement("input");
